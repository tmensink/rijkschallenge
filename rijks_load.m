function data = rijks_load(expOpts)
    ...
    fprintf('%30s | %7.4f | %s\n',mfilename,v,datestr(now,31));
		...
		if exist(expOpts.data.file,'dir'),
		  %% Add code, something like:
			Fdata.X = zeros(size(q.X,1),numel(encOpts.imdb.images.set),'single');

			T = {'TRAIN','VAL','TEST'},
			for t =1:numel(T),
				fprintf('T%d: %s\n',t,T{t});
				q = load([encOpts.pool.path '/feat_' T{t} '.mat']);
				inx = find(encOpts.imdb.images.set==t);
				Fdata.X(:,inx) = q.X;
			end
		else
